	...
	private void initView() {
		...
		if ("1".equals(oprate)) {
			...
			if (eventBean != null) {
				...
				//String aString=dTime.substring(0, 4) + "年" + dTime.substring(5, 7) + "月" +  dTime.substring(8, 10)+"日"+dTime.substring(10, 16);
				//endTime = aString;
	...
	private void initTimeDialogView(View view){
	 ...
	 private void initTypeDialogView(View view){
		// PickerView pvType=(PickerView) view.findViewById(R.id.pvType);
		 ...
		 if(defautType.equals("永不")){
			 rgGroup.check(R.id.cbYB);
		 }else if(defautType.equals("每天")){
			 rgGroup.check(R.id.cbMT);
		 }else if(defautType.equals("每月")){
			 rgGroup.check(R.id.cbMY);
		 }else if(defautType.equals("每周")){
	 ...
	 private String getTypeCode(String mtype){
		if(mtype.equals("永不")){
			return "0";
		}else if(mtype.equals("每天")){
			return "1";
		}else if(mtype.equals("每周")){
			return "2";
		}else if(mtype.equals("每月")){
			return "3";
		}
		return null;
	 }
	 ...
	 private void setUpData() {
			...
			String time=tvNoticeTime.getText().toString();
			if(time.length()>=17&&"永不".equals(type)){//type为永不。显示具体日期
			...
			}else if(time.length()==5&&"每天".equals(type)){//type为每天，显示时分
				cYear="";
				cMonth="";
				cDay="";
				cHour=time.substring(0,2)+"时";
				cMinute=time.substring(3,5)+"分";
			}else if(time.length()==9&&"每周".equals(type)){//type为每周，XX:XX 每周X
				cYear="";
				cMonth="";
				cDay=time.substring(1,3);
				cHour=time.substring(4,6)+"时";
				cMinute=time.substring(7,9)+"分";
			}else if(time.length()==11&&"每月".equals(type)){//type为每月。 XX:XX 每月XX日
				cYear="";
				cMonth="";
