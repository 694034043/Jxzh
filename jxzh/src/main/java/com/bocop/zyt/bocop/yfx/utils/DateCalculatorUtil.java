 ...
 * 
 */
package com.bocop.zyt.bocop.yfx.utils;
 ...
 * @version V 1.0
 * 
 *          实现的接口如下: 1---根据开始时间和间隔天数计算结束时间 Date calculateEndDate(Date sDate, int
 *          days) 2---根据开始时间、结束时间、间隔类型（年、月、日）计算间隔时间 int calInterval(Date sDate,
 *          Date eDate, String type) 3---输出日历实例（当前时间）的各个字段值 void
 *          printFields(Calendar cNow) 4---判定某个年份是否是闰年 boolean isLeapYear(int
 *          year) 5---比较两个日期的大小 int compareDate(Date sDate, Date eDate)
 */
