...
import com.bocop.zyt.bocop.xms.bean.QztHeadBody;
...
public class QztRequest implements LoginUtil.ILoginListener {
	...
	public QztRequest(Context context) {
	...
	public void postOpboc(String strUrl, CallBackBoc callBack) {
		...
		if (baseApplication.isNetStat()) {
			Log.i("tag", strUrl);
			post(strUrl,true,callBack);
		} else {
			CustomProgressDialog.showBocNetworkSetDialog(mContext);
		}
	}
	
	public void postOpbocNoDialog(String strUrl, CallBackBoc callBack) {

		if (baseApplication.isNetStat()) {
			Log.i("tag", strUrl);
			post(strUrl,false,callBack);
		} else {
			CustomProgressDialog.showBocNetworkSetDialog(mContext);
		}
	}
	 ...
	 */
	private RequestHandle post(String url, boolean isShowDialog, final CallBackBoc callBack) {
		...
		handle = client.post(url, new AsyncHttpResponseHandler() {
			...
			public void onSuccess(int statusCode, Header[] headers,
				...
				if(statusCode == 200){
					QztHeadBody qztHeadBody;
					QztHead qztHead;
					String head;
					String body;
					try {
						qztHeadBody = JsonUtils.getObject(strResponseBody,QztHeadBody.class);
						head = qztHeadBody.getHead();
						body = qztHeadBody.getBody();
						Log.i("tag2", head);
						Log.i("tag2", body);
						qztHead = JsonUtils.getObject(head, QztHead.class);
						if(qztHead.getStat().equals("00")){
							callBack.onSuccess(body);
							Log.i("tag2", "QztRequest call onSuccess msgcde ");
						}
						else{
							callBack.onFailure(qztHead.getResult());
