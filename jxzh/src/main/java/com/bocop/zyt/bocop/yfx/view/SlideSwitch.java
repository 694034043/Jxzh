...
public class SlideSwitch extends View {
	...
	private String textOff = "关";
	private String textOn = "开";
	...
	public SlideSwitch(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		FormsUtil.getDisplayMetrics(context);
		listener = null;
		paint = new Paint();
		...
		textOff = a.getString(R.styleable.slideswitch_textOff) == null ? "关"
				: a.getString(R.styleable.slideswitch_textOff);
		textOn = a.getString(R.styleable.slideswitch_textOn) == null ? "开"
				: a.getString(R.styleable.slideswitch_textOn);
		textSize = (int) a.getDimension(R.styleable.slideswitch_swTextSize, 16);
		a.recycle();
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int maxLength = textOff.length() > textOn.length() ? textOff.length() : textOn.length();

		int width = measureDimension((maxLength + 3) * textSize + 2 * RIM_SIZE, widthMeasureSpec);
		int height = measureDimension(textSize * 3, heightMeasureSpec);
		...
		if (shape == SHAPE_RECT) {
			...
			if (!isOpen) {
				canvas.drawText(textOff,
						backRect.height() + 2 * RIM_SIZE + (backRect.width() - (backRect.height() + 2 * RIM_SIZE)) / 2
								- (textSize * (textOff.length() + 1)) / 2,
						backRect.centerY() + textSize / 2, paint);
			} else {
				canvas.drawText(textOn, (backRect.width() - (backRect.height() + 2 * RIM_SIZE)) / 2
						- (textSize * (textOff.length() - 1)) / 2, backRect.centerY() + textSize / 2, paint);
			}
			paint.setColor(Color.WHITE);
