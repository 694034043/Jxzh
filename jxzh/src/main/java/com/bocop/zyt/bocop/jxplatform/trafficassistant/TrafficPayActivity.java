...
import com.bocop.zyt.bocop.jxplatform.bean.SA0052;
import com.bocop.zyt.bocop.jxplatform.bean.SA0075;
import com.bocop.zyt.bocop.jxplatform.bean.SA0075Card;
...
import com.bocop.zyt.bocop.jxplatform.xml.CspXmlForPay;
...
public class TrafficPayActivity extends BaseActivity {
	...
	TextView tvPayOrder;
	@ViewInject(R.id.tv_pay_peccancyNum)
	TextView tvPayPeccancyNum;
	@ViewInject(R.id.tv_pay_sum)
	TextView tvPeccancySum;			// 罚款金额
	@ViewInject(R.id.tv_pay_lateAmt)
	TextView tvLateAmt;			// 滞纳金
	@ViewInject(R.id.tv_pay_yjAmt)
	TextView tvYjAmt;			// 应缴金额
	@ViewInject(R.id.tv_pay_licenseNum)
...
//		initEvent();
	...
	private void requestBocopForCardBal() {
		...
		bocOpUtil.postOpboc(strGson, TransactionValue.SA0059, new BocOpUtil.CallBackBoc() {
...
//		List<Map<String,String>> list =new ArrayList<Map<String,String>>();
		...
		BocOpUtil bocOpUtil = new BocOpUtil(this);
		bocOpUtil.postOpboc(strGson,TransactionValue.SA0075, new BocOpUtil.CallBackBoc() {
...
//						cardArray[sa0075.getSaplist().size()] = "6259073676906698";
						...
						if (cardArrayStr[0].contains("信用卡")) {
							...
							Toast.makeText(TrafficPayActivity.this, "该缴费暂不支持信用卡支付", Toast.LENGTH_SHORT)
							.show();
					...
					e.printStackTrace();
					Toast.makeText(TrafficPayActivity.this, "意外事件发生，请稍候再试", Toast.LENGTH_LONG).show();
				}
			}
...
//					Toast.makeText(TrafficPayActivity.this, "请绑定借记卡进行缴费。", Toast.LENGTH_LONG).show();
	...
	private void initView() {
		...
		if(bundle != null){
			...
			strPeccancySum = bundle.getString("peccancySum");
			strLateAmt = bundle.getString("lateAmt");
			strYjAmt = bundle.getString("yjAmt");
			
			tvPayPeccancyNum.setText(strPeccancyNum);
			tvLicenseNum.setText(strLicenseNum);
			tvPeccancySum.setText(strPeccancySum);
			tvLateAmt.setText(strLateAmt);
			tvYjAmt.setText(strYjAmt);
	...
	public void btnAddBind(View v) {
		if (LoginUtil.isLog(TrafficPayActivity.this)){
			Log.i("tag", "BindCardActivity");
			Intent intent = new Intent(TrafficPayActivity.this, BocOpWebActivity.class);
	...
	public void requestBocForMsg() {
		...
		bocOpUtil.postOpboc(strGson,TransactionValue.SA0052, new BocOpUtilWithoutDia.CallBackBoc2() {
			SA0052 sa0052;
			@Override
			public void onSuccess(String responStr) {
				Log.i("tag", responStr);
				try {
					...
					if(sa0052.getResult().equals("0")){
						...
						Toast.makeText(TrafficPayActivity.this, "已发送至" + sa0052.mobleno + "，请查收", Toast.LENGTH_LONG).show();
					}
				} catch (Exception e) {
				...
				Toast.makeText(TrafficPayActivity.this, "短信验证码已发送！", Toast.LENGTH_SHORT);
			...
			public void onFailure(String responStr) {
				...
				if(responStr.equals("0") || responStr.equals("1") ){
					Toast.makeText(TrafficPayActivity.this, R.string.onFailure, Toast.LENGTH_SHORT).show();
				}
				else{
...
//				DialogUtils.NO_WAIT_DIALOG);
	...
	public void btPay(View v) {
		...
		Log.i("tag", "start1");
//		Log.i("tag", strYjAmt);
//		Log.i("tag", strCardBal);
		if(strYjAmt == null || strCardBal == null){
			Toast.makeText(TrafficPayActivity.this, "没有获取到卡余额", Toast.LENGTH_LONG).show();
			return;
		}
		if(Float.parseFloat(strYjAmt) > Float.parseFloat(strCardBal)){
			Toast.makeText(TrafficPayActivity.this, "您的卡余额不足", Toast.LENGTH_LONG).show();
			return;
		}
		if(checkMsg.length() == 6){
			...
			Toast.makeText(TrafficPayActivity.this, "请输入6位短信验证码", Toast.LENGTH_LONG).show();
		}
...
//			List<Map<String,String>> list =new ArrayList<Map<String,String>>();
			...
			BocOpUtil bocOpUtil = new BocOpUtil(this);
			bocOpUtil.postOpboc(strGson,TransactionValue.SA0056, new BocOpUtil.CallBackBoc() {
				...
				public void onSuccess(String responStr) {
					...
					try {
						...
						if(sa0052.getResult().equals("0")){
							...
							Toast.makeText(TrafficPayActivity.this, "短信验证码输入有误", Toast.LENGTH_LONG).show();
						}
					} catch (Exception e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
	...
	protected void requestCspForPay() {
		...
		try {
			//生成CSP XML报文
			CspXmlForPay cspXmlForPay = new CspXmlForPay(strPeccancyNum,choiseCard,strYjAmt,LoginUtil.getUserId(TrafficPayActivity.this));
			String strXml = cspXmlForPay.getCspXml();
			//生成MCIS报文
			Mcis mcis = new Mcis(strXml,TransactionValue.APJJ13);
			...
			cspUtil.postCspLogin(byteMessage, new CspUtil.CallBack() {
				@Override
				public void onSuccess(String responStr) {
					Log.i("tag", "返回包问体： " + responStr);
					try {
						CspRecHeaderBean cspRecHeader = CspRecHeader.readStringXml(responStr);
						if(cspRecHeader.getErrorcode().equals("00")){
							Toast.makeText(TrafficPayActivity.this, cspRecHeader.getErrormsg(), Toast.LENGTH_LONG).show();
							Bundle bundle = new Bundle();
							bundle.putString("peccancyNum", strPeccancyNum);
							bundle.putString("sjAmt", strYjAmt);
							Intent intent = new Intent(TrafficPayActivity.this, TrafficPaySuccessActivity.class);
							intent.putExtras(bundle);
							startActivity(intent);
						}
						else{
				...
				public void onFailure(String responStr) {
					if(responStr.equals("0")){
						Toast.makeText(TrafficPayActivity.this, "服务器错误，请稍候再试", Toast.LENGTH_SHORT).show();
					}else{
						Toast.makeText(TrafficPayActivity.this, responStr, Toast.LENGTH_SHORT).show();
					}
				}
