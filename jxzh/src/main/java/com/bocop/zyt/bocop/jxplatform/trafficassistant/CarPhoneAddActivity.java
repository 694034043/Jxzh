...
import com.bocop.zyt.bocop.jxplatform.xml.CspXmlAPJJ01;
...
@ContentView(R.layout.activity_trafficcarphoneadd)
public class CarPhoneAddActivity extends BaseActivity implements OnClickListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (bundle != null) {
			...
			strOwnerName = bundle.getString("name").toString();
			tvName.setText("姓名：" + strOwnerName);
			try{
				if(strIdNo.length() == 18){
					Log.i("tag", "18");
					tvId.setText("身份证号：" + strIdNo.substring(0, 6) + "********" + strIdNo.substring(14, 18));
				}else if (strIdNo.length() == 15){
					Log.i("tag", "15");
					tvId.setText("身份证号：" + strIdNo.substring(0, 6) + "******" + strIdNo.substring(11, 15));
				}else{
					tvId.setText("身份证号：" + strIdNo);
					Log.i("tag", "1");
				}
			}
			catch(Exception ex){
...
//								CustomProgressDialog.showBocRegisterSetDialog(CarAddActivity.this);
								...
								CustomProgressDialog.showBocRegisterSetDialog(CarPhoneAddActivity.this);
//								Toast.makeText(CarAddActivity.this, "请前往中银开发平台实名认证", Toast.LENGTH_LONG).show();
					...
					public void onFailure(String responStr) {
						CspUtil.onFailure(CarPhoneAddActivity.this, responStr);
					}
				});
	}
			...
			cspUtil.postCspLogin( byteMessage, new CspUtil.CallBack() {
				...
				public void onSuccess(String responStr) {
					...
					try {
						...
						if(cspRecHeaderBean.getErrorcode().equals("00")){
							Toast.makeText(CarPhoneAddActivity.this, "绑定成功", Toast.LENGTH_LONG).show();
							setResult(RESULT_OK);
							finish();
						}
						else{
							if(cspRecHeaderBean.getErrormsg().contains("手机号码与机动车登记手机号码不匹配")){
								Toast.makeText(CarPhoneAddActivity.this, "绑定失败，手机号码与车辆登记手机号码不匹配，请到车管所核实信息并进行修改", Toast.LENGTH_LONG).show();
							}else if(cspRecHeaderBean.getErrormsg().contains("姓名与机动车登记的所有人姓名不匹配")){
								Toast.makeText(CarPhoneAddActivity.this, "绑定失败，姓名与机动车登记的所有人姓名不匹配，请到车管所核实信息并进行修改", Toast.LENGTH_LONG).show();
							}
							else{
								Toast.makeText(CarPhoneAddActivity.this, cspRecHeaderBean.getErrormsg(), Toast.LENGTH_LONG).show();
							}
				...
				public void onFailure(String responStr) {
					if(responStr.equals("0")){
						Toast.makeText(CarPhoneAddActivity.this, "服务器错误，请稍候再试", Toast.LENGTH_SHORT).show();
					}else{
						Toast.makeText(CarPhoneAddActivity.this, responStr, Toast.LENGTH_SHORT).show();
					}
				}
			});
...
//				List<Map<String,String>> list =new ArrayList<Map<String,String>>();
				...
				BocOpUtil bocOpUtil = new BocOpUtil(this);
				bocOpUtil.postOpboc(strGson,TransactionValue.SA7115, new BocOpUtil.CallBackBoc() {
...
//							e.printStackTrace();
//						}
						Toast.makeText(CarPhoneAddActivity.this, "短信已发送，请查收", Toast.LENGTH_LONG).show();
						tvSendMsg.setText("59秒后重新获取");
						lltSendMsg.setClickable(false);
						handler.sendEmptyMessage(LAST_TIME);
						myProgressBar.removeView();
						rltLoading.setVisibility(View.GONE);
						Toast.makeText(CarPhoneAddActivity.this, "短信验证码已发送！", Toast.LENGTH_SHORT);
					...
					public void onFailure(String responStr) {
						...
						if(responStr.equals("0") || responStr.equals("1") ){
							Toast.makeText(CarPhoneAddActivity.this, R.string.onFailure, Toast.LENGTH_SHORT).show();
						}
						else{
								Toast.makeText(CarPhoneAddActivity.this,responStr , Toast.LENGTH_SHORT).show();
						}
