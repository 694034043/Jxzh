package com.bocop.zyt.bocop.gm.utils;
...
public class ContactsUtil {
	...
	public static List<ContentValues> findLinkmanInfo(Context context, String[] queryValues) {
		...
		ContentResolver resolver = context.getContentResolver();
		Cursor cursor = resolver.query(uri, new String[] { Data._ID, Contacts.DISPLAY_NAME,
				Contacts.HAS_PHONE_NUMBER, Contacts._ID }, null, null, null); //
		while (cursor.moveToNext()) {
			...
			if (cursor.getInt(2) > 0 && values.getAsString(PHONE) != null) {
				...
				Cursor phoneCursor = resolver.query(Phone.CONTENT_URI, null,
						Phone.CONTACT_ID + "=" + cursor.getInt(0), null, null);
	...
	public static List<ContactsInfo> getAllPhoneContacts(Context context) {
		...
		String[] mContactsProjection = new String[] { Phone.CONTACT_ID,
				Phone.NUMBER, Contacts.DISPLAY_NAME,
