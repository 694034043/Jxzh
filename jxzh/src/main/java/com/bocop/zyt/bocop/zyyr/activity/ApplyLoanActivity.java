...
public class ApplyLoanActivity extends BaseActivity {
	...
	@OnClick({ R.id.tvDeadLineChoser, R.id.tvGetCheckCode, R.id.btnNext })
	...
	private void calculateAndSet() {
		...
		if (!TextUtils.isEmpty(etLoanSum.getText().toString())) {
			...
			totalInterest = loanAmount * rate * period;
			refundPM = (loanAmount + totalInterest) / period;
			tvInterest.setText(DataFormatUtil.moneyStringFormat(totalInterest * 10000 + "") + "元");
			tvRefundPM.setText(DataFormatUtil.moneyStringFormat(refundPM * 10000 + "") + "元");
		}
	}
